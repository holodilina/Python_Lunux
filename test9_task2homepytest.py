...
# Fixture to write to stat.txt
def write_stat(file_path, info):
    with open(file_path, 'a') as file:
        file.write(info + '\n')

def test_fixture(tmp_path):
    # Create stat.txt file in tmp_path directory
    stat_file = tmp_path / "stat.txt"
    stat_file.touch()

    # Run test_stepl and write stat info to stat.txt
    test_stepl()
    write_stat(stat_file, "time1, files1, size1, stat1")

    # Run test_step2 and write stat info to stat.txt
    test_step2()
    write_stat(stat_file, "time2, files2, size2, stat2")

    # Run test_step3 and write stat info to stat.txt
    test_step3()
    write_stat(stat_file, "time3, files3, size3, stat3")

    # Run test_step4 and write stat info to stat.txt
    test_step4()
